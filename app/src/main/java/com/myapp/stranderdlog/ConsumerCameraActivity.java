...
public class ConsumerCameraActivity extends Activity implements View.OnClickListener {
    private static int crop_part = 1;
    ...
    String Meter_ID;
    Bitmap crop_4to=null,full_4to=null,crop_4to_id=null;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        userLocalStore = new UserLocalStore(this); SharedPreferences myPrefs2 = this.getSharedPreferences("myPrefs", MODE_WORLD_READABLE);

        Meter_ID = myPrefs2.getString("Meter_ID","");
    ...
    public void onClick(View v) {
        ...
        if(v.getId()==R.id.save_send_btn){
            ...
            if(full_4to !=null && crop_4to!=null && crop_4to_id !=null ){
                new uploadImage(full_4to, (Meter_ID + "_full")).execute();
                new uploadImage(crop_4to, (Meter_ID + "_crop")).execute();
                new uploadImage(crop_4to_id,(Meter_ID + "_crop_id")).execute();
                ...
                else if (crop_4to_id==null)
                    Toast.makeText(getApplicationContext(),"Error,crop_id image is not available", Toast.LENGTH_SHORT).show();
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (resultCode == RESULT_OK) {
            ...
            //user is returning from cropping the image
            else if(requestCode == PIC_CROP && crop_part==1){
                ...
                crop_part=2;
                Toast.makeText(getApplicationContext(),"crop Meter ID", Toast.LENGTH_LONG).show();
                performCropID();
            }
            else if(requestCode == PIC_CROP && crop_part==2){
                //get the returned data
                Bundle extras = data.getExtras();
                //get the cropped bitmap
                Bitmap thePic = extras.getParcelable("data");
                ...
                //retrieve a reference to the ImageView
                ImageView picView = (ImageView)findViewById(R.id.picture_crop_id);
                //display the returned cropped image
                picView.setImageBitmap(thePic);
            }
